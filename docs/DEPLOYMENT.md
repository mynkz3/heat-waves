...
For the shortest setup instructions, see the [README](../README.md).
Run all commands below from the project root, not from `docs/`.
...
Read [Data sources](DATA_SOURCES.md) for provenance, timestamps and caveats.
...
requirements; see [Data sources](DATA_SOURCES.md) and the license files in
`frontend/vendor/`.
